  ...
  ngOnInit(): void {
    ...
    this.jsonServer.getProducts().subscribe(products =>{
      products.forEach(oneProduct=>{
        if(oneProduct.categoryId==4){
          this.products.push(oneProduct)
          this.Allproducts.push(oneProduct)
          this.discountedPrice = (oneProduct.price - (oneProduct.price*(oneProduct.discount/100))).toFixed()
          oneProduct.discountPrice=this.discountedPrice
        }
      })
    })
  }
  ...
  radio(event:any){
    ...
    // }
      ...
      this.Allproducts.forEach(Oneproduct => {
        ...
        if(event.target.value == "2000 & Above"){
          if(Oneproduct.discountPrice > 2000){
            this.products.push(Oneproduct)
          }
        }
      })
